...
export class Oauth2Service {
  ...
  fetch(): CreateQueryResult<ConnectedUser> {
    return injectQuery(() => ({
      ...
      queryFn: () => firstValueFrom(this.fetchUserHttp()),
    }));
  }
  fetchUserHttp(): Observable<string> {
    return this.http.get<string>(`${environment.apiUrl}/users/test`);
  }
  /*fetchUserHttp(forceResync: boolean): Observable<ConnectedUser> {
    const params = new HttpParams().set('forceResync', forceResync);
    return this.http.get<ConnectedUser>(
      `${environment.apiUrl}/users/test`,
      { params }
    );
  }
*/
